...
ASSETS_PATH = OUTPUT_PATH / Path(r"table_4_assets\frame0")
...
def checking():
    ...
    if time == "AM":
        status = get_and_edit_value_in_excel(path_of_exel, "Table 4 AM", dat, slot_time)
        ...
        status = get_and_edit_value_in_excel(path_of_exel, "Table 4 PM", dat, slot_time)
        if status == "not available":
            tkinter.messagebox.showinfo("Welcome to GFG.", "No Seats Available.Please Book On Other Slot")
        elif status == "available":
            table_id = "table_2_pm"
